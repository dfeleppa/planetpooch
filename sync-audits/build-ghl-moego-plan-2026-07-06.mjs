...
const paths = {
  currentCsv: "C:/Users/Daniel/Downloads/MoeGo-Export-Client-PlanetPooch-2026-07-06-15-17-08.csv",
  lastAppliedCsv: "C:/Users/Daniel/Downloads/MoeGo-Export-Client-PlanetPooch-2026-06-26-13-51-50.csv",
  ...
  lastAppliedAudit: "C:/Users/Daniel/Documents/Planet Pooch/sync-audits/ghl-ui-update-results-2026-06-26.json",
  normalizedOut: "C:/Users/Daniel/Documents/Planet Pooch/sync-audits/moego-client-export-normalized-2026-07-06.json",
  conflictsOut: "C:/Users/Daniel/Documents/Planet Pooch/sync-audits/moego-client-export-phone-conflicts-2026-07-06.json",
  planOut: "C:/Users/Daniel/Documents/Planet Pooch/sync-audits/ghl-moego-delta-plan-2026-07-06.json",
};
...
function buildKnownGhlMap(list, audit) {
  ...
  for (const result of audit.results || []) {
    ...
    if (result.action === "updated" && result.phoneMatches) {
      const afterValue = result.after?.value ?? result.moegoTotalNumber ?? result.moegoTotal;
    ...
    } else if (/no_change/i.test(result.action || "") && result.phoneMatches !== false) {
      const beforeValue = result.before?.value ?? result.moegoTotalNumber ?? result.moegoTotal;
...
const [currentRowsRaw, lastAppliedRowsRaw, previousRowsRaw, ghlList, lastAppliedAudit] = await Promise.all([
  readMoegoCsv(paths.currentCsv),
  readMoegoCsv(paths.lastAppliedCsv),
  readMoegoCsv(paths.previousCsv),
  readJson(paths.ghlList),
  readJson(paths.lastAppliedAudit),
]);

const currentRows = normalizeMoegoRows(currentRowsRaw);
const lastAppliedRows = normalizeMoegoRows(lastAppliedRowsRaw);
const previousRows = normalizeMoegoRows(previousRowsRaw);
...
const plan = {
  ...
  currentRows: currentRowsRaw.length,
