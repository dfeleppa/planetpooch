...
async function replaceRows({
  ...
  const deleteRows = prisma.financeGoogleCampaignReportRow.deleteMany({
  ...
  if (rows.length === 0) {
    ...
    await prisma.$transaction([
      deleteRows,
      prisma.financeGoogleCampaignReportRow.createMany({
        data: rows.map((row) => ({
          business,
          periodStart,
          periodEnd,
  ...
  return prisma.financeGoogleCampaignReportRow.findMany({
    where: { business, periodStart, periodEnd },
...
export async function GET(req: NextRequest) {
  ...
  const rows = await prisma.financeGoogleCampaignReportRow.findMany({
    where: { business, periodStart, periodEnd },
    orderBy: { rowOrder: "asc" },
  });
