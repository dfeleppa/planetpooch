...
import { syncWeeklyTrainingKpis } from "@/lib/moego/training-weekly-report";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const report = await syncWeeklyTrainingKpis({
      weekStart,
    ...
    console.error("Training MoeGo KPI import failed:", err);
    return NextResponse.json(
      { error: `Training MoeGo KPI import failed: ${message}` },
      { status: 500 }
    );
  }
}
