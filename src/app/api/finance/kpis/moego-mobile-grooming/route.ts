...
import {
  syncWeeklyMobileGroomingKpis,
  MOBILE_GROOMING_BUSINESS_ID,
} from "@/lib/moego/mobile-grooming-weekly-report";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const report = await syncWeeklyMobileGroomingKpis({
      weekStart,
    ...
    console.error("Mobile grooming MoeGo KPI import failed:", err);
    return NextResponse.json(
      { error: `Mobile grooming MoeGo KPI import failed: ${message}` },
      { status: 500 }
    );
  }
}
