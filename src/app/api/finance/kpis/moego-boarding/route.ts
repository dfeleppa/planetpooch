...
import { syncWeeklyBoardingKpis } from "@/lib/moego/boarding-weekly-report";
...
export async function POST(req: NextRequest) {
  ...
  try {
    const report = await syncWeeklyBoardingKpis({
      weekStart,
    ...
    console.error("Boarding MoeGo KPI import failed:", err);
    return NextResponse.json(
      { error: `Boarding MoeGo KPI import failed: ${message}` },
      { status: 500 }
    );
  }
}
